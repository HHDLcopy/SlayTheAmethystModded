...
Part of the Gradle multi-project build. Produces `agent-connector.jar` with `Premain-Class` and `Agent-Class` manifest attributes.

The app's Gradle build copies this jar into `components/agent_connector/` in generated runtime assets. At launch, the game JVM receives `-javaagent:<path>=port=9099`.
