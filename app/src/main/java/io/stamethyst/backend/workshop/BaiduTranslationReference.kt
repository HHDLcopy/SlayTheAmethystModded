...
internal fun buildBaiduWorkshopCommentReference(
    modTitle: String,
    gameTitle: String,
): String =
    buildString {
        append("将内容视为 Steam 创意工坊模组评论进行翻译。")
        ...
        append(
            "请结合玩家评论语境，保持自然准确；优先保留游戏名、模组名、专有术语、角色名、道具名、技能名、版本号、文件路径、代码、链接、表情和原有格式，不要补充原文没有的信息。",
        )
    }.take(BAIDU_REFERENCE_LIMIT)
