...
`single-room` is a harness-owned autoplay run mode. The harness writes or forwards a properties spec, pushes it to the device when needed, starts MTS autoplay with `autoplayMode=single_room`, and leaves the process running after the runtime logs `[amethyst-autoplay] single_room result ...`. Run the generic `exit` command to request a GDX close through the launcher; `stop` remains a force-stop fallback. Startup success is reported as `SINGLE_ROOM_STARTED`.
