package io.stamethyst.probe.integration;
...
import io.stamethyst.probe.util.AsmMethodInterceptor;
...
public class AgentmainBytecodeTest {
    ...
    public void setUp() {
        ...
        registry.register("tracing", new MonitorRegistry.MonitorFactory() {
            @Override
            public io.stamethyst.probe.monitors.Monitor create(Instrumentation inst, String argsJson, AgentDataChannel channel) {
    ...
    private byte[] applyInterceptor(byte[] inputBytes, String internalName,
        ...
        if (!io.stamethyst.probe.monitors.impl.TracingMonitor.matchesPattern(dotName, dotPattern)) {
            return null;
        }
        try {
            ClassReader reader = new ClassReader(inputBytes);
