...
public class SpriteBatch implements Batch {
	...
	static public ShaderProgram createDefaultShader () {
		...
		ShaderProgram shader = tryCompile("default", vertexShader, fragmentShader, errors);
		if (shader != null) return shader;

		String vertex120 = "#version 120\n" //
			...
			+ "   gl_Position = u_projTrans * " + ShaderProgram.POSITION_ATTRIBUTE + ";\n" //
			+ "}\n";
		String fragment120 = "#version 120\n" //
			+ "varying vec4 v_color;\n" //
			+ "varying vec2 v_texCoords;\n" //
			+ "uniform sampler2D u_texture;\n" //
			+ "void main()\n" //
			...
			+ "  gl_FragColor = v_color * texture2D(u_texture, v_texCoords);\n" //
			+ "}\n";
		shader = tryCompile("glsl120", vertex120, fragment120, errors);
		if (shader != null) return shader;

		String vertex100 = "#version 100\n" //
			+ "attribute vec4 " + ShaderProgram.POSITION_ATTRIBUTE + ";\n" //
			+ "attribute vec4 " + ShaderProgram.COLOR_ATTRIBUTE + ";\n" //
			+ "attribute vec2 " + ShaderProgram.TEXCOORD_ATTRIBUTE + "0;\n" //
			+ "uniform mat4 u_projTrans;\n" //
			...
			+ "   gl_Position = u_projTrans * " + ShaderProgram.POSITION_ATTRIBUTE + ";\n" //
			+ "}\n";
		String fragment100 = "#version 100\n" //
			+ "precision mediump float;\n" //
			+ "varying vec4 v_color;\n" //
			+ "varying vec2 v_texCoords;\n" //
			+ "uniform sampler2D u_texture;\n" //
			...
			+ "  gl_FragColor = v_color * texture2D(u_texture, v_texCoords);\n" //
			+ "}\n";
		shader = tryCompile("glsl100", vertex100, fragment100, errors);
		if (shader != null) return shader;

		throw new IllegalArgumentException("Error compiling shader: " + errors.toString());
	}

	private static ShaderProgram tryCompile (String label, String vertexShader, String fragmentShader, StringBuilder errors) {
		...
		errors.append(label).append(": ").append(log.replace('\n', ' ').replace('\r', ' ').trim());
