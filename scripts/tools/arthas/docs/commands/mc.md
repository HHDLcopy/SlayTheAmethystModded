...
编译生成`.class`文件之后，可以结合[retransform](/doc/retransform.html)命令实现热更新代码。

注意

注意，mc 命令有可能失败。如果编译失败可以在本地编译好`.class`文件，再上传到服务器。具体参考[retransform](/doc/retransform.html)命令说明。

[在 GitHub 上编辑此页在新窗口打开](https://github.com/alibaba/arthas/edit/master/site/docs/doc/mc.md)

Last Updated:

贡献者: hengyunabc, Hollow Man, Hollow Man, vic

[mbean](/doc/mbean.html) [memory](/doc/memory.html)