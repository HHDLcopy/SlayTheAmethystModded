...
from .sts_harness import AGENT_COMMANDS, COMMANDS, LAUNCH_MODES, Harness, HarnessOptions
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Class name or Class#method to decompile. Repeatable. Example: com.megacrit.cardcrawl.cards.AbstractCard or com.megacrit.cardcrawl.cards.AbstractCard#applyPowers",
    )
    parser.add_argument("-AgentCommand", "--agent-command", dest="agent_command", choices=AGENT_COMMANDS, default="", help="Agent operation to run after smoke succeeds.")
    parser.add_argument("-AgentSpec", "--agent-spec", dest="agent_spec", default="", help="Monitor spec (e.g. tracing@classes=com.megacrit.*).")
    parser.add_argument("-AgentPort", "--agent-port", dest="agent_port", type=int, default=9090, help="TCP port for agent-connector.")
    parser.add_argument("-AgentDuration", "--agent-duration", dest="agent_duration", type=float, default=0, help="Seconds to capture agent data before detach.")
    parser.add_argument(
        "-RedefineClass",
        "--redefine-class",
